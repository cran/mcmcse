...
Before sampling the Markov chain, the user is advised to used the function \texttt{minESS} to figure out how many minimum effective samples are needed for analysis. See \cite{vats:fleg:jones:2015b} for details. Since we have two quantities, $\mu$ and $\mu_g$, we will find the minimum samples required for each analysis.
<<minESS>>= 
# For mu
...
\begin{itemize}
	...
	\item \texttt{eps} is the tolerance level. Default is .05. Reasonable levels are anywhere from .01 to .05. The smaller the tolerance, the larger the minimum effective samples.
\end{itemize}
...
We see that the effective samples is now larger than the minimum effective samples, so we know 28,000 are enough.
